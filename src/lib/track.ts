 ...
 * Events fire only after the visitor accepts cookies (eunik-cookies=1).
 */
...
function sendToApi(payload: Record<string, unknown>): void {
  ...
  // Blob + application/json so express.json() parses sendBeacon bodies
...
function fireEvent(
  ...
  // NDPR / cookie policy: measure traffic only after consent
